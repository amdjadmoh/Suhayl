...
export default function AddEditProgram(): React.ReactElement {
  ...
  // One-shot guard so the preselect runs at most once even if queries refetch.
  const preselectApplied = useRef(false);

  const { data: existing, isLoading } = useProgram(id ?? "");
  // Fetch the preselected university directly so we know its country immediately
  // — without requiring the user to manually pick a country first.
  const { data: preselectedUni } = useUniversity(preselectedUniId);
  ...
  // Pre-populate country + university as soon as the preselected university loads.
  useEffect(() => {
    if (!isEdit && preselectedUniId && preselectedUni && !preselectApplied.current) {
      setSelectedCountry(preselectedUni.country);
      setValue("universityId", preselectedUniId);
      preselectApplied.current = true;
    }
  }, [preselectedUni, preselectedUniId, isEdit, setValue]);
  ...
  async function handleCreateCustomUniversity(): Promise<void> {
    ...
    try {
      const created = await createUniversityMutation.mutateAsync({ name, country: selectedCountry, city });
  ...
  // True when the form was opened from a university page — lock the uni fields.
  const uniLocked = !isEdit && Boolean(preselectedUniId && preselectedUni);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="rounded-xl border border-border bg-card">
          ...
          <div className="p-6 grid gap-4 sm:grid-cols-2">
            ...
            {/* Country field — read-only when locked to a preselected university */}
            <div className="space-y-2 sm:col-span-2">
              <Label className="text-sm font-medium text-foreground">Country <span className="text-red-500">*</span></Label>
              {uniLocked ? (
                <div className="flex h-10 items-center rounded-lg border border-border bg-muted/50 px-3 text-sm text-foreground/70">
                  {preselectedUni!.country}
                </div>
              ) : (
                    ...
                    <div className="mt-2 rounded-lg border border-border bg-muted p-4 space-y-3">
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        <div className="space-y-1">
                          ...
                          <Input id="customUniName" value={customUniName} onChange={(e) => setCustomUniName(e.target.value)} placeholder="e.g. University of Oxford" className="rounded-lg border-border" />
                        </div>
                        <div className="space-y-1">
                          ...
                          <Input id="customUniCity" value={customUniCity} onChange={(e) => setCustomUniCity(e.target.value)} placeholder="e.g. Oxford" className="rounded-lg border-border" />
                        </div>
                      </div>
                      <div className="flex items-center justify-end gap-2 pt-1">
                        <Button type="button" variant="outline" onClick={() => { setShowCustomUniForm(false); setCustomUniName(""); setCustomUniCity(""); }} className="border-border hover:bg-muted rounded-xl">
                          Cancel
                        </Button>
                        <Button type="button" onClick={handleCreateCustomUniversity} disabled={!customUniName.trim() || !customUniCity.trim() || createUniversityMutation.isPending} className="bg-foreground text-background hover:bg-foreground/90 rounded-xl">
                          {createUniversityMutation.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Adding...</> : <><Plus className="mr-2 h-4 w-4" />Add University</>}
                        </Button>
                      </div>
                    </div>
                  )}
            ...
            <div className="space-y-2">
              ...
              <Select value={watch("degreeLevel")} onValueChange={(v) => setValue("degreeLevel", v as ProgramFormData["degreeLevel"])}>
            ...
            <div className="sm:col-span-3 space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                <Select value={selectedTest} onValueChange={(v) => { setSelectedTest(v); if (v !== "Custom") setCustomTestName(""); }}>
                  <SelectTrigger className="rounded-lg w-40"><SelectValue placeholder="Select test..." /></SelectTrigger>
                  <SelectContent>
                ...
                {selectedTest === "Custom" && (
                  <Input value={customTestName} onChange={(e) => setCustomTestName(e.target.value)} placeholder="Test name..." className="rounded-lg border-border w-40" />
                )}
                <Input type="number" value={newTestScore} onChange={(e) => setNewTestScore(e.target.value)} placeholder="Min score" className="rounded-lg border-border w-32" />
                <Button type="button" variant="outline" className="border-border hover:bg-muted rounded-xl" onClick={addTestRequirement}
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {testReqs.map((t, i) => (
                    <span key={i} className="bg-muted text-foreground rounded-full px-3 py-1 text-sm inline-flex items-center gap-1">
                      ...
                      <button type="button" onClick={() => removeTestRequirement(i)} className="ml-1 rounded-full hover:text-red-500"><X className="h-3 w-3" /></button>
                    </span>
                  ))}
                </div>
              )}
          ...
          <div className="p-6 grid gap-4 sm:grid-cols-2">
              ...
              {requiredDocuments.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-2">
                  {requiredDocuments.map((doc, i) => (
                    <span key={i} className="bg-muted text-foreground rounded-full px-3 py-1 text-sm inline-flex items-center gap-1">
                      {doc}
                      <button type="button" onClick={() => removeDocument(i)} className="ml-1 rounded-full hover:text-red-500"><X className="h-3 w-3" /></button>
                    </span>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-2 sm:col-span-2">
              <div className="flex items-center gap-2">
                <Checkbox id="scholarship" checked={scholarshipAvailable} onCheckedChange={(c) => setValue("scholarshipAvailable", c === true)} />
                <Label htmlFor="scholarship" className="text-sm font-medium text-foreground">Scholarship Available</Label>
              </div>
              {scholarshipAvailable && (
                <Textarea id="scholarshipDetails" className="rounded-lg border-border focus:border-primary focus:ring-primary/20" {...register("scholarshipDetails")} placeholder="Describe available scholarships..." rows={2} />
              )}
            </div>
            ...
            <div className="space-y-2 sm:col-span-2">
              <div className="grid gap-4 sm:grid-cols-3 pt-2 border-t border-border">
                <div className="flex items-center gap-2">
                  <Checkbox id="requiresSOP" checked={watch("requiresSOP") || false} onCheckedChange={(c) => setValue("requiresSOP", c === true)} />
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <Button type="submit" disabled={isSubmitting} className="bg-foreground text-background hover:bg-foreground/90 rounded-xl">
            {isSubmitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Saving...</> : <><Save className="mr-2 h-4 w-4" />{isEdit ? "Update" : "Save"} Program</>}
          </Button>
        </div>
      </form>
    </div>
