...
export default function UserDetail(): React.ReactElement {
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <AlertCircle className="mb-4 h-12 w-12 text-red-500" />
        <h2 className="text-lg font-semibold text-[#0F172A]">User not found</h2>
        <p className="text-sm text-slate-500">
  ...
  return (
    <div className="space-y-6">
      ...
      <button
        onClick={() => navigate("/users")}
        className="inline-flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-[#0F172A] transition-colors"
      >
            ...
            {userUnis.map((u) => (
              ...
              >
                <Link
                  ...
                  className="text-sm font-medium text-[#0F172A] hover:text-[#0EA5E9] transition-colors"
                >
